define(require => {
    ...
    function init() {
        ...
        coverAnimTimeline.to({}, cards[0].spineAnim.spineData.animations[6].duration, {
            onComplete: () => {
                cards.forEach(card => {
                    if(scenario) {
                        if (card.revealed === false && card.spineAnim.state.tracks[0].animation.name !== 'YourNumber_MOUSEOVER') {
        ...
        infoTimeLine = new Timeline({ repeat: -1 });

        infoTimeLine.add(Tween.to(displayList.matchThreeToWinText,0.5,{alpha:1}), 3);
        infoTimeLine.add(Tween.to(displayList.revealToText,0.5,{alpha:0}), 3);
        infoTimeLine.add(Tween.to(displayList.coin1,0.5,{alpha:0}), 3);
        infoTimeLine.add(Tween.to(displayList.coin2,0.5,{alpha:0}), 3);
        infoTimeLine.add(Tween.to(displayList.revealToText,0.5,{alpha:1}), 6);
        infoTimeLine.add(Tween.to(displayList.coin1,0.5,{alpha:1}), 6);
        infoTimeLine.add(Tween.to(displayList.coin2,0.5,{alpha:1}), 6);
        infoTimeLine.add(Tween.to(displayList.matchThreeToWinText,0.5,{alpha:0}), 6);
    ...
    function enable() {
        ...
        return cards.map(async card => {
            ...
            if (!autoPlay.enabled) {
                ...
                audio.playSequential('AutoplayNumber', false, 0.4);
            }
            ...
            msgBus.publish('Game.CheckIfTwoHaveMatched',card.symbolLetter);
    ...
    function revealAll() {
        ...
        cardRows[2] ? cardRows[2].forEach(c => { if (!c.revealed) c2.push(Tween.delayedCall(0, c.reveal, null, c)); }) : null;
    ...
    function populateBonusWinAtEndOfGame() {
        haltUnmatched();

        cards.forEach(e=>{
            if(e.symbolCode === 'X') {
                e.winAmount.text = bonusWins.coin;
            }
            if(e.symbolCode === 'Y') {
                e.winAmount.text = bonusWins.wheel;
            }
        });
    }

    function setBonusWinValues(data) {
        if(data.game === 'coin') {
            bonusWins.coin = SKBeInstant.formatCurrency(data.value).formattedAmount;
        }
        if(data.game === 'wheel') {
            bonusWins.wheel = SKBeInstant.formatCurrency(data.value).formattedAmount;
        }
    }
    ...
    function checkIfTwoHaveMatched(data) {

        if (cards.filter(e => e.symbolLetter === data).length === 2) {
            if (!autoPlay.enabled) {

                cards.filter(e => e.symbolLetter === data).forEach(e => {
                    if(!e.matched) {
                        let symbolAnimName = e.symbolName.toUpperCase() +"/"+ e.symbolName;

                        if(e.revealed && e.symbol.spine.state.tracks[0].animation.name !== symbolAnimName+'MatchIdle2') {
                            e.symbol.spine.state.setAnimation(0, symbolAnimName+'MatchIdle2', true);
                        }
                    }
                });
            }
    ...
    function haltUnmatched() {
        cards.forEach(e=>{
            if(!e.matched) {
                let symbolAnimName = e.symbolName.toUpperCase() +"/"+ e.symbolName;

                if(e.symbol.spine.state.tracks[0].animation.name === symbolAnimName+'MatchIdle2') {
                    e.symbol.spine.stateData.setMix(symbolAnimName+'MatchIdle2',symbolAnimName+'Idle',0.5);
                    e.symbol.spine.state.setAnimation(0, symbolAnimName+'Idle', true);
                }
            }
        });
    }
    ...
    msgBus.subscribe('Game.ShowWinAnim',()=>{
        displayList.landscapeBG.children[0].state.timeScale = 3;
        displayList.portraitBG.children[0].state.timeScale = 3;
        displayList.coinLandscapeBG.children[0].state.timeScale = 3;
        displayList.coinPortraitBG.children[0].state.timeScale = 3;
        displayList.wbSpine.children[0].state.timeScale = 3;
    });

    msgBus.subscribe('Game.ResetWinAnim',()=>{
        displayList.landscapeBG.children[0].state.timeScale = 0.8;
        displayList.portraitBG.children[0].state.timeScale = 0.8;
        displayList.coinLandscapeBG.children[0].state.timeScale = 0.8;
        displayList.coinPortraitBG.children[0].state.timeScale = 0.8;
        displayList.wbSpine.children[0].state.timeScale = 0.8;
    });
    ...
    msgBus.subscribe('Game.PopulateBonusWinAtEndOfGame',populateBonusWinAtEndOfGame);
    msgBus.subscribe('Game.SetBonusWinValues',setBonusWinValues);

    msgBus.subscribe('Game.CheckIfTwoHaveMatched',checkIfTwoHaveMatched);
